...
// EXMO is a major Russian cryptocurrency exchange
// Documentation: https://exmo.com/en/api_doc

pub const EXMO = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, auth_config: auth.AuthConfig, testnet: bool) !*EXMO {
        const self = try allocator.create(EXMO);
        self.allocator = allocator;
        self.api_key = auth_config.apiKey;
        self.secret_key = auth_config.apiSecret;
        self.testnet = testnet;

        // EXMO uses decimal_places precision mode
        ...
        const base_name = try allocator.dupe(u8, "exmo");
        const base_url = try allocator.dupe(u8, "https://api.exmo.com");
        const ws_url = try allocator.dupe(u8, "wss://api.exmo.com/ws/");
    ...
    pub fn fetchMarkets(self: *EXMO) ![]Market {
        _ = self;
        return error.NotImplemented;
    }

    pub fn fetchTicker(self: *EXMO, symbol: []const u8) !Ticker {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchOrderBook(self: *EXMO, symbol: []const u8, limit: ?u32) !OrderBook {
        _ = self;
        _ = symbol;
        _ = limit;
        return error.NotImplemented;
    ...
    pub fn fetchOHLCV(self: *EXMO, symbol: []const u8, timeframe: []const u8, since: ?i64, limit: ?u32) ![]OHLCV {
        _ = self;
        _ = symbol;
        _ = timeframe;
        _ = since;
    ...
    pub fn fetchTrades(self: *EXMO, symbol: []const u8, since: ?i64, limit: ?u32) ![]Trade {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
    ...
    pub fn fetchBalance(self: *EXMO) ![]Balance {
        _ = self;
        return error.NotImplemented;
    }

    pub fn createOrder(self: *EXMO, symbol: []const u8, order_type: OrderType, side: OrderSide, amount: f64, price: ?f64, params: ?std.StringHashMap([]const u8)) !Order {
        _ = self;
        _ = symbol;
        _ = order_type;
        _ = side;
    ...
    pub fn cancelOrder(self: *EXMO, order_id: []const u8, symbol: ?[]const u8) !void {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
    ...
    pub fn fetchOrder(self: *EXMO, order_id: []const u8, symbol: ?[]const u8) !Order {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
    ...
    pub fn fetchOpenOrders(self: *EXMO, symbol: ?[]const u8) ![]Order {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchClosedOrders(self: *EXMO, symbol: ?[]const u8, since: ?i64, limit: ?u32) ![]Order {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
...
pub fn create(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*EXMO {
    return EXMO.init(allocator, auth_config, false);
}

pub fn createTestnet(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*EXMO {
    return EXMO.init(allocator, auth_config, true);
}