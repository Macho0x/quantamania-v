...
// Indodax is a major Indonesian cryptocurrency exchange
// Documentation: https://indodax.com/

pub const Indodax = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, auth_config: auth.AuthConfig, testnet: bool) !*Indodax {
        const self = try allocator.create(Indodax);
        self.allocator = allocator;
        self.api_key = auth_config.apiKey;
        self.secret_key = auth_config.apiSecret;
        self.testnet = testnet;

        // Indodax uses decimal_places precision mode
        ...
        const base_name = try allocator.dupe(u8, "indodax");
        const base_url = try allocator.dupe(u8, "https://indodax.com");
    ...
    pub fn deinit(self: *Indodax) void {
    ...
    pub fn fetchMarkets(self: *Indodax) ![]Market {
        _ = self;
        return error.NotImplemented;
    }

    pub fn fetchTicker(self: *Indodax, symbol: []const u8) !Ticker {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchOrderBook(self: *Indodax, symbol: []const u8, limit: ?u32) !OrderBook {
        _ = self;
        _ = symbol;
        _ = limit;
        return error.NotImplemented;
    ...
    pub fn fetchOHLCV(self: *Indodax, symbol: []const u8, timeframe: []const u8, since: ?i64, limit: ?u32) ![]OHLCV {
        _ = self;
        _ = symbol;
        _ = timeframe;
        _ = since;
    ...
    pub fn fetchTrades(self: *Indodax, symbol: []const u8, since: ?i64, limit: ?u32) ![]Trade {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
    ...
    pub fn fetchBalance(self: *Indodax) ![]Balance {
        _ = self;
        return error.NotImplemented;
    }

    pub fn createOrder(self: *Indodax, symbol: []const u8, order_type: OrderType, side: OrderSide, amount: f64, price: ?f64, params: ?std.StringHashMap([]const u8)) !Order {
        _ = self;
        _ = symbol;
        _ = order_type;
        _ = side;
    ...
    pub fn cancelOrder(self: *Indodax, order_id: []const u8, symbol: ?[]const u8) !void {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
    ...
    pub fn fetchOrder(self: *Indodax, order_id: []const u8, symbol: ?[]const u8) !Order {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
    ...
    pub fn fetchOpenOrders(self: *Indodax, symbol: ?[]const u8) ![]Order {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchClosedOrders(self: *Indodax, symbol: ?[]const u8, since: ?i64, limit: ?u32) ![]Order {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
...
pub fn create(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*Indodax {
    return Indodax.init(allocator, auth_config, false);
}

pub fn createTestnet(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*Indodax {
    return Indodax.init(allocator, auth_config, true);
}