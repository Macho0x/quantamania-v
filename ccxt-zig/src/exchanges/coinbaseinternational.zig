...
pub const CoinbaseInternational = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, auth_config: auth.AuthConfig, testnet: bool) !*CoinbaseInternational {
        ...
        const base_url = try allocator.dupe(u8, "https://api.coinbase.com");
        const ws_url = try allocator.dupe(u8, "wss://ws-feed.exchange.coinbase.com");
    ...
    pub fn deinit(self: *CoinbaseInternational) void {
    ...
    pub fn fetchTicker(self: *CoinbaseInternational, symbol: []const u8) !Ticker {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }
    ...
    pub fn fetchBalance(self: *CoinbaseInternational) ![]Balance {
        _ = self;
        return error.NotImplemented;
    }
};
...
pub fn create(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*CoinbaseInternational {
    return CoinbaseInternational.init(allocator, auth_config, false);
}

pub fn createTestnet(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*CoinbaseInternational {
    return CoinbaseInternational.init(allocator, auth_config, true);
}