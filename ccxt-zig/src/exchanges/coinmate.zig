...
// Coinmate is a European cryptocurrency exchange
// Documentation: https://coinmate.io/docs

pub const Coinmate = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, auth_config: auth.AuthConfig, testnet: bool) !*Coinmate {
        const self = try allocator.create(Coinmate);
        self.allocator = allocator;
        self.api_key = auth_config.apiKey;
        self.secret_key = auth_config.apiSecret;
        self.testnet = testnet;

        // Coinmate uses decimal_places precision mode
        ...
        const base_name = try allocator.dupe(u8, "coinmate");
        const base_url = try allocator.dupe(u8, "https://coinmate.io");
    ...
    pub fn deinit(self: *Coinmate) void {
    ...
    pub fn fetchMarkets(self: *Coinmate) ![]Market {
        _ = self;
        return error.NotImplemented;
    }

    pub fn fetchTicker(self: *Coinmate, symbol: []const u8) !Ticker {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchOrderBook(self: *Coinmate, symbol: []const u8, limit: ?u32) !OrderBook {
        _ = self;
        _ = symbol;
        _ = limit;
        return error.NotImplemented;
    ...
    pub fn fetchOHLCV(self: *Coinmate, symbol: []const u8, timeframe: []const u8, since: ?i64, limit: ?u32) ![]OHLCV {
        _ = self;
        _ = symbol;
        _ = timeframe;
        _ = since;
    ...
    pub fn fetchTrades(self: *Coinmate, symbol: []const u8, since: ?i64, limit: ?u32) ![]Trade {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
    ...
    pub fn fetchBalance(self: *Coinmate) ![]Balance {
        _ = self;
        return error.NotImplemented;
    }

    pub fn createOrder(self: *Coinmate, symbol: []const u8, order_type: OrderType, side: OrderSide, amount: f64, price: ?f64, params: ?std.StringHashMap([]const u8)) !Order {
        _ = self;
        _ = symbol;
        _ = order_type;
        _ = side;
    ...
    pub fn cancelOrder(self: *Coinmate, order_id: []const u8, symbol: ?[]const u8) !void {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
    ...
    pub fn fetchOrder(self: *Coinmate, order_id: []const u8, symbol: ?[]const u8) !Order {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
    ...
    pub fn fetchOpenOrders(self: *Coinmate, symbol: ?[]const u8) ![]Order {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchClosedOrders(self: *Coinmate, symbol: ?[]const u8, since: ?i64, limit: ?u32) ![]Order {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
...
pub fn create(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*Coinmate {
    return Coinmate.init(allocator, auth_config, false);
}

pub fn createTestnet(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*Coinmate {
    return Coinmate.init(allocator, auth_config, true);
}