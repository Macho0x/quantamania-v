...
fn main() {
    ...
    if validated_config := ema_config.validate() {
        ema_result := indicators.ema(test_data_1, validated_config)
        if ema_result.len > 0 {
    ...
    if validated_config := bollinger_config.validate() {
        upper, middle, lower := indicators.bollinger_bands(test_data_1, validated_config)
    ...
    if validated_config := rsi_config.validate() {
        rsi_result := indicators.rsi(test_data_2, validated_config)
        if rsi_result.len > 0 {
    ...
    if validated_config := adx_config.validate() {
        adx_line, plus_di, minus_di := indicators.adx(test_data_3, validated_config)
    ...
    if validated_config := macd_config.validate() {
        macd_line, signal_line, histogram := indicators.macd(test_data_3, validated_config)
    ...
    if validated_config := sar_config.validate() {
        sar_result := indicators.parabolic_sar(test_data_3, validated_config)
        if sar_result.len > 0 {
    ...
    if validated_config := ichimoku_config.validate() {
        tenkan, kijun, senkou_a, senkou_b, chikou := indicators.ichimoku_cloud(test_data_3, validated_config)
    ...
    if validated_config := stochastic_config.validate() {
        k_line, d_line := indicators.stochastic_oscillator(test_data_3, validated_config)
    ...
    if validated_config := williams_config.validate() {
        williams_result := indicators.williams_percent_r(test_data_3, validated_config)
    ...
    if validated_config := cci_config.validate() {
        cci_result := indicators.cci(test_data_3, validated_config)
        if cci_result.len > 0 {
    ...
    if validated_config := cmf_config.validate() {
        cmf_result := indicators.chaikin_money_flow(test_data_3, validated_config)
        if cmf_result.len > 0 {
    ...
    if validated_config := atr_config.validate() {
        atr_result := indicators.average_true_range(test_data_3, validated_config)
        if atr_result.len > 0 {
    ...
    if validated_config := keltner_config.validate() {
        keltner_upper, keltner_middle, keltner_lower := indicators.keltner_channels(test_data_3, validated_config)
    ...
    if validated_config := awesome_config.validate() {
        awesome_result := indicators.awesome_oscillator(test_data_3, validated_config)
    ...
    if validated_config := ultimate_config.validate() {
        ultimate_result := indicators.ultimate_oscillator(test_data_3, validated_config)
    ...
    if validated_config := trix_config.validate() {
        trix_result := indicators.trix(test_data_3, validated_config)
    ...
    if validated_config := donchian_config.validate() {
        donchian_upper, donchian_middle, donchian_lower := indicators.donchian_channels(test_data_3, validated_config)
    ...
    if validated_config := wilders_config.validate() {
        wilders_result := indicators.wilders_smoothing(test_data_3, validated_config)
    ...
    if validated_config := roc_config.validate() {
        roc_result := indicators.rate_of_change(test_data_3, validated_config)
        if roc_result.len > 0 {
    ...
    if validated_config := mfi_config.validate() {
        mfi_result := indicators.money_flow_index(test_data_3, validated_config)
        if mfi_result.len > 0 {
    ...
    if validated_config := stoch_rsi_config.validate() {
        stoch_rsi_k, stoch_rsi_d := indicators.stochastic_rsi(test_data_3, validated_config)
    ...
    if validated_config := kst_config.validate() {
        kst_line, kst_signal := indicators.know_sure_thing(test_data_3, validated_config)
    ...
    if validated_config := coppock_config.validate() {
        coppock_result := indicators.coppock_curve(test_data_3, validated_config)
    ...
    if validated_config := vortex_config.validate() {
        plus_vi, minus_vi := indicators.vortex_indicator(test_data_3, validated_config)
    ...
    if validated_config := hma_config.validate() {
        hma_result := indicators.hull_moving_average(test_data_3, validated_config)
        if hma_result.len > 0 {
    ...
    if validated_config := linear_reg_config.validate() {
        linear_result := indicators.linear_regression(test_data_3, validated_config)
    ...
    if validated_config := vwma_config.validate() {
        vwma_result := indicators.volume_weighted_moving_average(test_data_3, validated_config)
    ...
    if validated_config := dema_config.validate() {
        dema_result := indicators.double_exponential_moving_average(test_data_3, validated_config)
    ...
    if validated_config := tema_config.validate() {
        tema_result := indicators.triple_exponential_moving_average(test_data_3, validated_config)
    ...
    if validated_config := ama_config.validate() {
        ama_result := indicators.adaptive_moving_average(test_data_3, validated_config)
        if ama_result.len > 0 {
    ...
    if validated_config := alma_config.validate() {
        alma_result := indicators.arnaud_legoux_moving_average(test_data_3, validated_config)
    ...
    if validated_config := momentum_config.validate() {
        momentum_result := indicators.momentum(test_data_3, validated_config)
    ...
    if validated_config := proc_config.validate() {
        proc_result := indicators.price_rate_of_change(test_data_3, validated_config)
    ...
    if validated_config := dpo_config.validate() {
        dpo_result := indicators.detrended_price_oscillator(test_data_3, validated_config)
        if dpo_result.len > 0 {
    ...
    if validated_config := ppo_config.validate() {
        ppo_result := indicators.percentage_price_oscillator(test_data_3, validated_config)
    ...
    if validated_config := cmo_config.validate() {
        cmo_result := indicators.chande_momentum_oscillator(test_data_3, validated_config)
        if cmo_result.len > 0 {
    ...
    if validated_config := fisher_config.validate() {
        fisher_result := indicators.fisher_transform(test_data_3, validated_config)
    ...
    eom_config := indicators.EOMConfig{period: 14}
    if validated_config := eom_config.validate() {
        eom_result := indicators.ease_of_movement(test_data_3, validated_config)
        if eom_result.len > 0 {
    ...
    if validated_config := std_dev_config.validate() {
        std_dev_result := indicators.standard_deviation(test_data_3, validated_config)
    ...
    if validated_config := hist_vol_config.validate() {
        hist_vol_result := indicators.historical_volatility(test_data_3, validated_config)
    ...
    if validated_config := chaikin_vol_config.validate() {
        chaikin_vol_result := indicators.chaikin_volatility(test_data_3, validated_config)
    ...
    if validated_config := ulcer_config.validate() {
        ulcer_result := indicators.ulcer_index(test_data_3, validated_config)
    ...
    if validated_config := gator_config.validate() {
        gator_result := indicators.gator_oscillator(test_data_3, validated_config)
    ...
    if validated_config := volume_roc_config.validate() {
        volume_roc_result := indicators.volume_rate_of_change(test_data_3, validated_config)
    ...
    if validated_config := vpt_config.validate() {
        vpt_result := indicators.volume_price_trend(test_data_3, validated_config)
        if vpt_result.len > 0 {
    ...
    if validated_config := nvi_config.validate() {
        nvi_result := indicators.negative_volume_index(test_data_3, validated_config)
        if nvi_result.len > 0 {
    ...
    if validated_config := pvi_config.validate() {
        pvi_result := indicators.positive_volume_index(test_data_3, validated_config)
        if pvi_result.len > 0 {
    ...
    if validated_config := mfv_config.validate() {
        mfv_result := indicators.money_flow_volume(test_data_3, validated_config)
        if mfv_result.len > 0 {
    ...
    if validated_config := enhanced_vwap_config.validate() {
        enhanced_vwap_result := indicators.enhanced_vwap(test_data_3, validated_config)
    ...
    if validated_config := pivot_config.validate() {
        pivot_result := indicators.pivot_points(test_data_3, validated_config)
    ...
    if validated_config := fib_config.validate() {
        fib_result := indicators.fibonacci_retracements(test_data_3, validated_config)
    ...
    if validated_config := price_channels_config.validate() {
        price_channels_result := indicators.price_channels(test_data_3, validated_config)
    ...
    if validated_config := andrews_config.validate() {
        andrews_result := indicators.andrews_pitchfork(test_data_3, validated_config)
    ...
    println('  • Total: 57 indicators tested')
    println('')
