...
// Mercado Bitcoin Exchange (Brazil)
...
pub const MercadoBitcoin = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, auth_config: auth.AuthConfig, testnet: bool) !*MercadoBitcoin {
        const self = try allocator.create(MercadoBitcoin);
        ...
        const base_name = try allocator.dupe(u8, "mercado");
        const base_url = try allocator.dupe(u8, "https://www.mercadobitcoin.net/api");
        const ws_url = try allocator.dupe(u8, "wss://ws.mercadobitcoin.net");
    ...
    pub fn fetchMarkets(self: *MercadoBitcoin) ![]Market {
        _ = self;
        return error.NotImplemented;
    }

    pub fn fetchTicker(self: *MercadoBitcoin, symbol: []const u8) !Ticker {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchOrderBook(self: *MercadoBitcoin, symbol: []const u8, limit: ?u32) !OrderBook {
        _ = self;
        _ = symbol;
        _ = limit;
        return error.NotImplemented;
    ...
    pub fn fetchTrades(self: *MercadoBitcoin, symbol: []const u8, since: ?i64, limit: ?u32) ![]Trade {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
    ...
    pub fn fetchBalance(self: *MercadoBitcoin) ![]Balance {
        _ = self;
        return error.NotImplemented;
    }

    pub fn createOrder(self: *MercadoBitcoin, symbol: []const u8, order_type: OrderType, side: OrderSide, amount: f64, price: ?f64, params: ?std.StringHashMap([]const u8)) !Order {
        _ = self;
        _ = symbol;
        _ = order_type;
        _ = side;
    ...
    pub fn cancelOrder(self: *MercadoBitcoin, order_id: []const u8, symbol: ?[]const u8) !void {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
...
pub fn create(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*MercadoBitcoin {
    return MercadoBitcoin.init(allocator, auth_config, false);
}

pub fn createTestnet(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*MercadoBitcoin {
    return MercadoBitcoin.init(allocator, auth_config, true);
}
