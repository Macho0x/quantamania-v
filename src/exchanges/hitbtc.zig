...
pub const HitBTC = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, auth_config: auth.AuthConfig, testnet: bool) !*HitBTC {
        const self = try allocator.create(HitBTC);
        ...
        const base_url = try allocator.dupe(u8, if (testnet) "https://api.hitbtc.com" else "https://api.hitbtc.com");
        const ws_url = try allocator.dupe(u8, "wss://api.hitbtc.com/api/2/ws");
    ...
    pub fn fetchMarkets(self: *HitBTC) ![]Market {
        _ = self;
        return error.NotImplemented;
    }

    pub fn fetchTicker(self: *HitBTC, symbol: []const u8) !Ticker {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchOrderBook(self: *HitBTC, symbol: []const u8, limit: ?u32) !OrderBook {
        _ = self;
        _ = symbol;
        _ = limit;
        return error.NotImplemented;
    ...
    pub fn fetchTrades(self: *HitBTC, symbol: []const u8, since: ?i64, limit: ?u32) ![]Trade {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
    ...
    pub fn fetchBalance(self: *HitBTC) ![]Balance {
        _ = self;
        return error.NotImplemented;
    }

    pub fn createOrder(self: *HitBTC, symbol: []const u8, order_type: OrderType, side: OrderSide, amount: f64, price: ?f64, params: ?std.StringHashMap([]const u8)) !Order {
        _ = self;
        _ = symbol;
        _ = order_type;
        _ = side;
    ...
    pub fn cancelOrder(self: *HitBTC, order_id: []const u8, symbol: ?[]const u8) !void {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
...
pub fn create(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*HitBTC {
    return HitBTC.init(allocator, auth_config, false);
}

pub fn createTestnet(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*HitBTC {
    return HitBTC.init(allocator, auth_config, true);
}
