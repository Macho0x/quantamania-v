...
// Bitso Exchange (Mexico / LatAm)
// Documentation: https://bitso.com/api_info
pub const BitSO = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, auth_config: auth.AuthConfig, testnet: bool) !*BitSO {
        const self = try allocator.create(BitSO);
        ...
        const base_name = try allocator.dupe(u8, "bitso");
        const base_url = try allocator.dupe(u8, if (testnet) "https://api-staging.bitso.com/v3" else "https://api.bitso.com/v3");
        const ws_url = try allocator.dupe(u8, if (testnet) "wss://ws-staging.bitso.com" else "wss://ws.bitso.com");
    ...
    pub fn fetchMarkets(self: *BitSO) ![]Market {
        _ = self;
        return error.NotImplemented;
    }

    pub fn fetchTicker(self: *BitSO, symbol: []const u8) !Ticker {
        _ = self;
        _ = symbol;
        return error.NotImplemented;
    }

    pub fn fetchOrderBook(self: *BitSO, symbol: []const u8, limit: ?u32) !OrderBook {
        _ = self;
        _ = symbol;
        _ = limit;
        return error.NotImplemented;
    ...
    pub fn fetchTrades(self: *BitSO, symbol: []const u8, since: ?i64, limit: ?u32) ![]Trade {
        _ = self;
        _ = symbol;
        _ = since;
        _ = limit;
    ...
    pub fn fetchBalance(self: *BitSO) ![]Balance {
        _ = self;
        return error.NotImplemented;
    }

    pub fn createOrder(self: *BitSO, symbol: []const u8, order_type: OrderType, side: OrderSide, amount: f64, price: ?f64, params: ?std.StringHashMap([]const u8)) !Order {
        _ = self;
        _ = symbol;
        _ = order_type;
        _ = side;
    ...
    pub fn cancelOrder(self: *BitSO, order_id: []const u8, symbol: ?[]const u8) !void {
        _ = self;
        _ = order_id;
        _ = symbol;
        return error.NotImplemented;
...
pub fn create(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*BitSO {
    return BitSO.init(allocator, auth_config, false);
}

pub fn createTestnet(allocator: std.mem.Allocator, auth_config: auth.AuthConfig) !*BitSO {
    return BitSO.init(allocator, auth_config, true);
}
